...
Description:	Updating proc to pull size from new tblBuyAuctionVehicleAttributes Table 

Updated By:		Gwendolyn Lukemire
Updated On:		10/10/2018
Description:	Updating proc to pull PurchaseChannel from tblBuyAuctionPurchaseAttribute
***********************************************************************************/
...
BEGIN
    BEGIN TRY
        ...
        FROM mybuys.tblBuyAuctionFee baf
            JOIN mybuys.tblBuyAuctionFeeType ft
        ...
        SELECT --b.BuyAuctionVehicleID,
            ...
            bv.Make,
            bv.Model,
            Odometer = ISNULL(bava.BuyerOdometer,bava.Odometer),
            Size = ISNULL(bava.Size,bava.BuyerSize),
            b.BuyerName,
            b.BuyerEmployeeID,
            elt.SourcingRegion,
            elt.CRLLT,
            ...
            BidTypeKey = CASE
                             ...
                             ELSE
                                 'UNDEFINED'
                         END,
            PurchaseChannel = bapct.PurchaseChannelKey,
        ...
        FROM dbo.tblBuyAuctionVehicle bav
			JOIN dbo.tblBuyAuctionVehicleAttributes bava
				ON bava.BuyAuctionVehicleID = bav.BuyAuctionVehicleID
			...
			LEFT JOIN mybuys.tblBuyAuctionPurchaseAttribute bapa ----Added 10/10
				ON bapa.BuyAuctionVehicleID = bav.BuyAuctionVehicleID
			LEFT JOIN mybuys.tblBuyAuctionPurchaseChannelType bapct ----Added 10/10
				ON bapct.BuyAuctionPurchaseChannelTypeID = bapa.BuyAuctionPurchaseChannelTypeID;
...
-- SELECT * FROM #SemiFinal
		...
		--EXEC [dbo].[stpInsertUpdateBuyNetEntry]
        --SELECT * FROM Buy.dbo.tblBuyNetEntry WHERE ISDELETED = 1

----UPDATE DELETED RECORDS
UPDATE bne
SET IsDeleted = 1
--DECLARE @DateFilter DATE SET @DateFilter = DATEADD(d, -11, CAST(GETDATE() AS DATE)) SELECT bne.* 
FROM dbo.tblBuyNetEntry bne
    JOIN dbo.tblBuyVehicle bv
        ON bv.VIN = bne.VIN
    JOIN dbo.tblBuyAuctionVehicle bav
        ON bav.BuyVehicleID = bv.BuyVehicleID
    JOIN dbo.tblBuyAuctionListType balt
        ON balt.BuyAuctionListTypeID = bav.BuyAuctionListTypeID
    JOIN dbo.tblBuy b
        ON b.BuyAuctionVehicleID = bav.BuyAuctionVehicleID
WHERE bne.PurchaseDate > @DateFilter --DATEADD(d,-11,CAST(GETDATE() AS DATE))
      AND bne.IsDeleted = 0
      AND (
              b.IsReconciledDateTime IS NULL
              OR bav.DestinationConfirmationDateTime IS NULL
          );
